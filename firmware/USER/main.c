...
typedef enum {
    MODE_AUTO = 0,    // 超声波自主避障
    MODE_MANUAL = 1,  // ROS 串口控制
} op_mode_t;

static op_mode_t g_mode = MODE_AUTO;
static uint32_t g_last_ros_cmd_tick = 0;
#define ROS_TIMEOUT_TICK  166   // ~5秒 (166 × 30ms)
...
static void update_leds(void)
{
    // D4=AUTO  D3=MANUAL
    if (g_mode == MODE_AUTO) {
        LED_D4_SET;  LED_D3_RESET;
    } else {
        LED_D4_RESET; LED_D3_SET;
    }
}

static void beep_ms(int ms)
{
    BEEP_SET; delay_ms(ms); BEEP_RESET;
}

static void button_scan(void)
{
    static uint8_t key_prev = 1;  // 上拉输入，默认高电平

    uint8_t key_now = KEY;  // PC2，按下=0
    if (key_prev == 1 && key_now == 0)  // 下降沿
    {
        // 切换模式
        if (g_mode == MODE_AUTO) {
            g_mode = MODE_MANUAL;
...
static void process_command(void)
{
    ...
    // 收到运动指令时切换到手动模式并刷新计时
    // 'S' 单独出现（停车）不计入 — 不阻止切回 AUTO
    if (buf[0] == 'F' || buf[0] == 'B' || buf[0] == 'L' ||
        buf[0] == 'R' ||
        (buf[0] == 'S' && len >= 2 && (buf[1] == 'L' || buf[1] == 'R')))
    {
        g_mode = MODE_MANUAL;
        g_last_ros_cmd_tick = 0;
    }
    ...
    switch (buf[0])
    {
        ...
        case 'S':
            if (len >= 2 && buf[1] == 'L') {
                ...
                // AUTO 模式下忽略 S（桥接持续发 S 会覆盖避障电机指令）
                if (g_mode == MODE_AUTO) break;
                cmd_stop();
                printf("OK S\r\n");
            }
            break;
...
int main(void)
{
    ...
    update_leds();  // 初始：D4亮=AUTO
    beep_ms(150);

    printf("\r\n=== ZYSTM32-A1 SONIC+ROS ===\r\n");
    printf("Mode: AUTO (D4 LED on)\r\n");
    printf("Press KEY to switch | M/A/H serial commands\r\n");
    ...
    while (1)
    {
        ...
        if (g_mode == MODE_MANUAL)
        {
            g_last_ros_cmd_tick++;
            // 每秒打印一次倒计时（串口可见）
            if (g_last_ros_cmd_tick % 33 == 0)
            {
                int remain = (ROS_TIMEOUT_TICK - g_last_ros_cmd_tick) / 33;
                if (remain > 0)
                    printf("! MANUAL timeout in %ds\r\n", remain);
            }
        ...
        // ── 3. 自主避障（仅在 AUTO 模式）──
        static op_mode_t prev_mode = MODE_AUTO;
        if (g_mode == MODE_AUTO)
        {
            // 刚从 MANUAL 切过来 → 重置避障状态
